...
describe('The CalendarMonth Component', function() {
  ...
  it('should render the right element', function() {
    ...
    expect(this.renderedComponent.props.className).toEqual(
  ...
  describe('has a label acting as a header', function() {
    ...
    it('which is a div with the correct class', function() {
      ...
      expect(this.container.props.className).toEqual(
    ...
    describe('displaying month information', function() {
      it('which creates a span with the correct class', function() {
        ...
        expect(span.props.className).toEqual(
          'DateRangePicker__MonthHeaderLabel DateRangePicker__MonthHeaderLabel--month'
        );
      });
      ...
      it('which calls props.onMonthChange if props.disableNavigation is false and if the selected value changes', function() {
        ...
        var select = TestUtils.scryRenderedDOMComponentsWithTag(
    ...
    describe('displaying year information', function() {
      it('which creates a span with the correct class', function() {
        ...
        expect(span.type).toBe('span');
        expect(span.props.className).toEqual(
          'DateRangePicker__MonthHeaderLabel DateRangePicker__MonthHeaderLabel--year'
        );
      });
    ...
    describe('has a table', function() {
      it('which has the expected className', function() {
        ...
        expect(this.table.props.className).toEqual(
          'DateRangePicker__MonthDates'
        );
      });

      it('whose head contains day information', function() {
        expect(this.table.props.children[0].props.children).toEqual(
          <tr className="DateRangePicker__Weekdays">
            <th
      ...
      it('which has a body containing the weeks', function() {
        expect(
          this.table.props.children[1].props.children.length
        ).toBeGreaterThan(3);
        expect(this.table.props.children[1].props.children[0].type).toEqual(
          'tr'
        );
        expect(
          this.table.props.children[1].props.children[1].props.children.length
        ).toBe(7);
        expect(
          this.table.props.children[1].props.children[1].props.children[0].type
        ).toBe(CalendarDate);
      });
    });
  });
